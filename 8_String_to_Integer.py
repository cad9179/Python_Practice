...
class Solution:
    def myAtoi(self, str: str) -> int:
        import re
        # str.strip() will remove the leading and trailing whitespace 
...
# '^[+\-]?\d+'
# ^ matches the beginning of the string ( go from the beginning of the string)  
# [] is character set, matches any single character inside the bracket,
# \d find all digit characters 
# + means preceding character \d appear one or more 


# The whole expression is (^[\+\-0]*\d+)\D*, 
# ^ matches the beginning of the string ( go from the beginning of the string)  
# [] is character set, matches any single character inside the bracket. In sets, +, *, ., |, (), $,{} has no special meaning. They just mean themselves
# * means preceding character [\+\-0] appear zero or more times,
# \d find all digit characters 
# + means preceding character \d appear one or more 
# \D means nondigits.
